...
async function deployFunction() {
  ...
  // IMPORTANT: Project ref for API calls vs hostname for URLs are different!
  const projectRef = process.env.SUPABASE_PROJECT_REF || 'your-project-ref';  // For Management API calls
  const hostname = process.env.SUPABASE_HOSTNAME || 'your-hostname';   // For frontend URLs
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`🔗 Function URL: https://${hostname}.supabase.co/functions/v1/orchestrator`);
    } else {
...
async function setEnvironmentVariables() {
  ...
  const envVars = [
    { name: 'SUPABASE_URL', value: `https://${hostname}.supabase.co` },
